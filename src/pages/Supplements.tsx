...
function SupplementsContent() {
  ...
  const handleTabChange = (tab: TabType) => { click(); setActiveTab(tab); };

  if (isLoading) return <ModuleIntroScreen icon={Pill} title="Supplements" subtitle="Loading your supplements" theme="supplements" />;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-supplements/5 pb-24">
      <div className="fixed top-0 left-0 right-0 z-50 border-b border-border/20 bg-gradient-to-r from-background/95 via-background/90 to-background/95 backdrop-blur-2xl">
        ...
        <div className="flex items-center justify-between p-4 relative">
          <div className="flex items-center gap-3">
            <Link to="/">
              <Button variant="ghost" size="icon" onClick={() => click()} className="hover:bg-supplements/10 hover:text-supplements transition-all duration-300 rounded-xl h-9 w-9">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold bg-gradient-to-r from-foreground via-foreground to-foreground/70 bg-clip-text text-transparent">Supplements</h1>
                <p className="text-[10px] text-muted-foreground font-medium tracking-wide uppercase">{supplements.length} tracked</p>
              </div>
            </div>
          </div>
          {lowStockCount > 0 && (
      ...
      <div className="p-4 space-y-4 pt-24 animate-fade-in max-w-2xl mx-auto">
        <LowStockAlert />
        {activeTab === 'calendar' ? <SupplementCalendar /> : <SupplementList />}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50">
        ...
        <div className="bg-background/80 backdrop-blur-2xl border-t border-border/20 shadow-2xl shadow-black/10">
          ...
          <div className="flex max-w-md mx-auto">
            {([{ tab: 'calendar' as TabType, icon: Calendar, label: 'Calendar' }, { tab: 'supplements' as TabType, icon: List, label: 'Supplements' }]).map(({ tab, icon: Icon, label }) => (
              <button key={tab} onClick={() => handleTabChange(tab)}
                className={cn("relative flex-1 flex flex-col items-center gap-0.5 py-4 transition-all duration-300 rounded-2xl mx-2 my-1",
                  activeTab === tab ? "text-supplements" : "text-muted-foreground hover:text-foreground")}>
                {activeTab === tab && (
                  <>
                ...
                <div className={`relative z-10 transition-all duration-300 ${activeTab === tab ? 'scale-110 -translate-y-0.5' : ''}`}>
                  <Icon className="h-5 w-5" />
                </div>
                <span className={`relative z-10 text-[10px] font-medium ${activeTab === tab ? 'font-semibold' : ''}`}>{label}</span>
              </button>
            ))}
          </div>
        </div>
...
export default function Supplements() {
  return <SupplementProvider><SupplementsContent /></SupplementProvider>;
}
