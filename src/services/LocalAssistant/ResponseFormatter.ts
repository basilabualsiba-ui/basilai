// Response Formatter - Formats query results into natural language with table support
...
export class ResponseFormatter {
  ...
  ): { text: string; tableData?: { headers: string[]; rows: any[][] } } {
    if (result.error) {
      return { text: `❌ حصل خطأ: ${result.error}` };
    }

    // Check for custom result code output
    if ((result as any)._formattedResult) {
      return { text: (result as any)._formattedResult };
    }

    const text = this.formatText(query, result, variables);

    // Generate table data if output_mode is table
    if (query.output_mode === 'table' && result.data.length > 0) {
      const tableData = this.generateTableData(result.data);
  ...
  private formatText(query: SavedQuery, result: QueryResult, variables: Record<string, any>): string {
  ...
  private applyTemplate(template: string, result: QueryResult, variables: Record<string, any>): string {
    let output = template;
  ...
  private generateResponse(query: SavedQuery, result: QueryResult, variables: Record<string, any>): string {
    ...
    switch (query.category) {
      case 'financial': return this.formatFinancialResponse(query, result, variables);
      case 'gym': return this.formatGymResponse(query, result, variables);
      case 'prayer': return this.formatPrayerResponse(query, result);
      case 'supplements': return this.formatSupplementsResponse(query, result);
      case 'dreams': return this.formatDreamsResponse(query, result);
      case 'schedule': return this.formatScheduleResponse(query, result);
      default: return this.formatGenericResponse(result);
    }
  }

  private generateTableData(data: any[]): { headers: string[]; rows: any[][] } {
    const excludeKeys = ['_rows', 'id', 'created_at', 'updated_at'];
    const headers = Object.keys(data[0]).filter(k => !excludeKeys.includes(k) && typeof data[0][k] !== 'object').slice(0, 6);
    const rows = data.slice(0, 20).map(row => headers.map(h => {
      const val = row[h];
      if (val === null) return '-';
      if (typeof val === 'number') return this.formatNumber(val, 'currency');
      return String(val);
    }));
    return { headers, rows };
  }

  // ... all the same category formatters as before
  private formatFinancialResponse(query: SavedQuery, result: QueryResult, variables: Record<string, any>): string {
    const { data, aggregations } = result;
    let response = '';
    ...
    if (data.length > 0 && query.query_name.includes('spending')) {
      ...
      if (data.length > 5) response += `... و${data.length - 5} معاملات أخرى`;
    }
    return response || this.formatGenericResponse(result);
  }

  private formatGymResponse(query: SavedQuery, result: QueryResult, variables: Record<string, any>): string {
    const { data, aggregations } = result;
    if (query.query_name.includes('workout') && query.query_name.includes('today')) {
      if (data.length === 0) return '💪 لا يوجد تمرين مجدول لليوم - يوم راحة! 🧘';
      const workout = data[0];
      const muscles = Array.isArray(workout.muscle_groups) ? workout.muscle_groups.join(' + ') : workout.muscle_groups;
      return `💪 **تمرين اليوم:**\n${muscles}\n${workout.start_time ? `⏰ الساعة ${workout.start_time}` : ''}`;
    }
    if (query.query_name.includes('weight')) {
      if (data.length === 0) return '⚖️ لا توجد قياسات وزن محفوظة';
      const latest = data[0];
    ...
    if (aggregations?.count !== undefined) return `🏋️ عدد التمارين: ${aggregations.count}`;
    return this.formatGenericResponse(result);
  }

  private formatPrayerResponse(query: SavedQuery, result: QueryResult): string {
    const { data } = result;
    if (data.length === 0) return '🕌 لا توجد مواقيت صلاة محفوظة لليوم';
    const times = data[0];
  ...
  private formatSupplementsResponse(query: SavedQuery, result: QueryResult): string {
    const { data } = result;
    if (data.length === 0) return '💊 لا توجد مكملات محفوظة';
  ...
  private formatDreamsResponse(query: SavedQuery, result: QueryResult): string {
    const { data } = result;
    if (data.length === 0) return '🌟 لا توجد أهداف نشطة';
    let response = '🌟 **أهدافك:**\n';
  ...
  private formatScheduleResponse(query: SavedQuery, result: QueryResult): string {
    const { data } = result;
    if (data.length === 0) return '📅 لا توجد أنشطة مجدولة لليوم';
  ...
  private formatGenericResponse(result: QueryResult): string {
    const { data, aggregations } = result;
    if (aggregations?.total !== undefined) return `📊 النتيجة: ${this.formatNumber(aggregations.total, 'currency')}`;
    if (aggregations?.count !== undefined) return `📊 العدد: ${aggregations.count}`;
    if (data.length > 0) return `📊 تم العثور على ${data.length} نتيجة`;
    return '📊 لا توجد نتائج';
  }

  private getEmptyResponse(category: string): string {
    const responses: Record<string, string> = {
      financial: '💰 لا توجد معاملات', gym: '💪 لا توجد بيانات تمارين',
      prayer: '🕌 لا توجد بيانات صلاة', supplements: '💊 لا توجد مكملات',
      dreams: '🌟 لا توجد أهداف', schedule: '📅 لا توجد أنشطة',
    };
    return responses[category] || '📊 لا توجد نتائج';
  }

  private formatNumber(num: number, type: 'currency' | 'count' | 'percentage'): string {
    if (type === 'currency') return num.toLocaleString('ar-SA', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
